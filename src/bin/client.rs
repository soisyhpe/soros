...
fn create_protocol_client() -> Result<ProtocolClient, ProtocolClientError> {
    ...
    let primary_hostname: String = args[1].parse().expect("Invalid primary hostname");
    let primary_port: u16 = args[2].parse().expect("Invalid primary port number");
    let secondary_hostname: String = args[3].parse().expect("Invalid secondary hostname");
    let secondary_port: u16 = args[4].parse().expect("Invalid secondary port number");

    ProtocolClient::new(&primary_hostname, primary_port, &secondary_hostname, secondary_port)
}
